...
import { Car, LayoutDashboard, History, Home, Moon, Sun, Menu, X, BrainCircuit } from 'lucide-react';
...
const Layout = () => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'dark:bg-slate-950 dark:text-slate-50' : 'bg-slate-50 text-slate-900'} flex flex-col font-sans`}>
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-50 shadow-sm">
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 animate-in slide-in-from-top-4 duration-300">
      ...
      <footer className="bg-white dark:bg-slate-900 py-10 border-t border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4 text-sm text-slate-400">
          <div className="flex items-center gap-2">
            <Car size={16} />
            <span className="font-bold text-slate-600 dark:text-slate-500">AutoClass ML</span>
          </div>
